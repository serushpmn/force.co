...
if (
  ...
  isset($_POST["edit_account_nonce"])
) {
  if (!wp_verify_nonce($_POST["edit_account_nonce"], "edit_account_action")) {
    ...
    if (empty($display_name)) {
      $errors[] = "نام نمایشی نمی‌تواند خالی باشد.";
    }
    if (empty($phone)) {
      $errors[] = "تلفن تماس نمیتواند خالی باشد.";
    }
    ...
    if (empty($errors)) {
      ...
      $user_updates = ["ID" => $current_user->ID];
      if (!empty($first_name)) {
      ...
      if (!empty($new_password)) {
        $user_updates["user_pass"] = $new_password;
      }
    ...
    <main>
      ...
      <form method="post" class="edit-account-form">
        ...
        <div class="grid two-col">
          <label>نام
            <input type="text" name="first_name" value="<?php echo esc_attr(
              $current_user->first_name,
            ); ?>" />
          </label>

          <label>نام خانوادگی
            <input type="text" name="last_name" value="<?php echo esc_attr(
              $current_user->last_name,
            ); ?>" />
          </label>

          <label>نام نمایشی
            <input type="text" name="display_name" value="<?php echo esc_attr(
              $current_user->display_name,
            ); ?>" />
          </label>

          <label>ایمیل
            <input type="text" name="email" value="<?php echo esc_attr(
        ...
        <div class="grid two-col">
          <label>شرکت
            <input type="text" name="billing_company" value="<?php echo esc_attr(
              get_user_meta($current_user->ID, "billing_company", true),
            ); ?>" />
          </label>

          <label>شهر
            <input type="text" name="billing_city" value="<?php echo esc_attr(
              get_user_meta($current_user->ID, "billing_city", true),
            ); ?>" />
          </label>

          <label>استان
            <input type="text" name="billing_state" value="<?php echo esc_attr(
              get_user_meta($current_user->ID, "billing_state", true),
            ); ?>" />
          </label>

          <label>کد پستی
            <input type="text" name="billing_postcode" value="<?php echo esc_attr(
              get_user_meta($current_user->ID, "billing_postcode", true),
            ); ?>" />
          </label>

          <label>تلفن
            <input type="text" name="billing_phone" value="<?php echo esc_attr(
              get_user_meta($current_user->ID, "billing_phone", true),
            ); ?>" />
          </label>

          <label>آدرس
            <textarea name="billing_address_1"><?php echo esc_textarea(
        ...
        <div style="margin-top:14px; display:flex; gap:12px;">
          ...
          <a href="<?php echo esc_url(
            home_url("/account"),
          ); ?>" class="button" style="background:#f3f4f6;color:#111;padding:10px 12px;border-radius:8px;text-decoration:none">انصراف</a>
        </div>
      </form>
